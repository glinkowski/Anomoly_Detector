...
vFiles = [(vDir + f) for f in listdir(vDir)]
vFiles.sort()
vFiles = vFiles[::-1]


# Perform detection on the files
for inVidName in vFiles :
	print("\nFinding vectors in {}".format(inVidName))

	tstart = time.time()


	# Read in the video file and initialize HOG
